    ...
    const result = await pool.query(
      `UPDATE boutiques SET
       ...
       WHERE user_id = $5 RETURNING *,
         (SELECT '/api/medias/' || logo_media_id WHERE logo_media_id IS NOT NULL) AS logo_url`,
...
app.get('/api/boutique/settings', require('./middleware/auth'), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT b.*,
         CASE WHEN b.logo_media_id IS NOT NULL THEN '/api/medias/' || b.logo_media_id ELSE NULL END AS logo_url
       FROM boutiques b WHERE b.user_id = $1 LIMIT 1`,
      [req.user.id]
    );
...
// ══ APPARENCE BOUTIQUE (couleurs) ══
app.put('/api/boutique/apparence', require('./middleware/auth'), async (req, res) => {
  const { couleur_primaire, couleur_secondaire } = req.body;
  const pool = require('./config/db');
  const hex = /^#[0-9A-Fa-f]{6}$/;
  if (couleur_primaire && !hex.test(couleur_primaire))   return res.status(400).json({ message: 'Couleur primaire invalide.' });
  if (couleur_secondaire && !hex.test(couleur_secondaire)) return res.status(400).json({ message: 'Couleur secondaire invalide.' });
  try {
    const result = await pool.query(
      `UPDATE boutiques SET
        couleur_primaire   = COALESCE($1, couleur_primaire),
        couleur_secondaire = COALESCE($2, couleur_secondaire),
        updated_at = NOW()
       WHERE user_id = $3 RETURNING id, couleur_primaire, couleur_secondaire`,
      [couleur_primaire || null, couleur_secondaire || null, req.user.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ message: 'Boutique introuvable.' });
    return res.json({ message: 'Couleurs sauvegardées.', boutique: result.rows[0] });
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({ message: 'Erreur serveur.' });
  }
...
// ══ LOGO BOUTIQUE (upload → stocké en base via medias) ══
...
app.post('/api/boutique/logo', require('./middleware/auth'), logoUpload.single('logo'), async (req, res) => {
  ...
  try {
    const mediaResult = await pool.query(
      ...
      [req.file.buffer, req.file.mimetype, req.file.size, req.file.originalname, req.user.id]
    );
    const mediaId = mediaResult.rows[0].id;

    // Ancien logo à nettoyer (évite d'accumuler des images orphelines en base)
    const previous = await pool.query('SELECT logo_media_id FROM boutiques WHERE user_id = $1', [req.user.id]);
    const oldLogoId = previous.rows[0]?.logo_media_id;

    await pool.query(
      `UPDATE boutiques SET logo_media_id = $1, updated_at = NOW() WHERE user_id = $2`,
      [mediaId, req.user.id]
    );

    if (oldLogoId) {
      await pool.query('DELETE FROM medias WHERE id = $1', [oldLogoId]);
    }

    return res.status(201).json({ message: 'Logo mis à jour.', logo_url: `/api/medias/${mediaId}` });
  } catch (err) {
