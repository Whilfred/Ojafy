...
async function sendConfirmationCommande({ email, nom_client, reference, produit, montant, boutique_nom, vendeur_tel }) {
  ...
  const suiviUrl = `http://localhost:3002/suivi?ref=${reference}`;
...
async function sendNotificationVendeur({ email_vendeur, nom_vendeur, reference, produit, montant, nom_client, telephone_client }) {
  ...
  const content = `
    ...
    <div style="text-align:center;">
      <a href="http://localhost:3002" style="display:inline-block;background:#1A6BFF;color:#ffffff;padding:14px 28px;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600;">📊 Voir le dashboard</a>
    </div>`;
...
async function sendEmailBienvenue({ email, prenom, boutique_slug }) {
  ...
  const content = `
    ...
    <div style="text-align:center;">
      <a href="http://localhost:3002/boutique/${boutique_slug}" style="display:inline-block;background:#1A6BFF;color:#ffffff;padding:14px 28px;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600;">🏪 Voir ma boutique</a>
    </div>`;
